...
# 
# ## Execution Order
# 1. Run `collect_capacity_metrics` first (to populate the calendar timepoints).
# 2. Run `generate_gold_tables` after this notebook to generate gold tables.
# 
# ## NULL Unique_key Policy
# Rows with `NULL` Unique_key ARE written to silver (for audit). When gold tables are
# generated by `generate_gold_tables`, NULL-key rows are **excluded** from gold.
...
logger = logging.getLogger('collect_timepoint_detail')
...
temp_store_state = create_temp_persisted_store('collect_timepoint_detail')
...
try:
    ...
    raise RuntimeError(
        ...
        f'Run shared_config (or run collect_capacity_metrics followed by generate_gold_tables) first. Error: {ex}'
    )
...
if prereq_count == 0:
    raise RuntimeError(
        ...
        f'Run shared_config (or run collect_capacity_metrics followed by generate_gold_tables) to populate {timepoint_calendar_table} first.'
    )
