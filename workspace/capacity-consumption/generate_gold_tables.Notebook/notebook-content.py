...
# This notebook is the single source of gold writes — run it after any collector (collect_capacity_metrics/collect_timepoint_detail/collect_autoscale_timepoint_detail).
# It is fully idempotent: safe to re-run at any time.
# 
# ## Inputs (silver — written by collectors)
# | Silver Table | Collector | Content |
# |---|---|---|
# | `silver.capacities` | collect_capacity_metrics | Raw capacity list |
# | `silver.capacity_metrics_timepoints` | collect_capacity_metrics | Daily per-timepoint CU metrics |
# | `silver.timepoint_metrics` | collect_timepoint_detail | Per-timepoint activity detail (standard) |
# | `silver.timepoint_metrics_autoscale` | collect_autoscale_timepoint_detail | Per-timepoint activity detail (autoscale) |
# 
...
# 
# ## Execution Order
# Run **after** one or more of: collect_capacity_metrics, collect_timepoint_detail, collect_autoscale_timepoint_detail.
# Optional silver tables (timepoint detail collection/autoscale timepoint detail collection outputs) are skipped gracefully if not yet populated.
...
logger = logging.getLogger('generate_gold_tables')
...
# (all targets are defined in shared_config)
...
logger.info('generate_gold_tables COMPLETE — all gold tables up to date')
