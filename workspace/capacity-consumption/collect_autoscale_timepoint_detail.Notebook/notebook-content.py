...
# # collect_autoscale_timepoint_detail
# 
# ## Purpose
...
# 
# ## Inputs
# - `shared_config` — all config
# - `gold.capacity_calendar_timepoints` — prerequisite: must exist (run collect_capacity_metrics first)
# 
# ## Outputs
...
# 
# ## Execution Order
# 1. `collect_capacity_metrics` (populates calendar timepoints)
# 2. This notebook (independent of timepoint detail collection — can run separately or together)
# 3. `generate_gold_tables` (generates gold tables including cumulative CU)
# 
# ## Bug Fixes vs. original collect_autoscale_timepoint_detail
...
# This is intentional and documented in generate_gold_tables.
...
logger = logging.getLogger('collect_autoscale_timepoint_detail')
...
temp_store_state = create_temp_persisted_store('collect_autoscale_timepoint_detail')
...
try:
    ...
    raise RuntimeError(f'PREREQUISITE FAILED: {timepoint_calendar_table} not accessible. Run shared_config (or run collect_capacity_metrics followed by generate_gold_tables) first. {ex}')
if prereq_count == 0:
    raise RuntimeError(f'PREREQUISITE FAILED: No timepoints for {target_capacity_id} in {start_date_str}--{end_date_str}. Run shared_config (or run collect_capacity_metrics followed by generate_gold_tables) first.')
